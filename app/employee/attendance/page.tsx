...
export default function EmployeeAttendancePage() {
  ...
  const selectableRows = useMemo(() => {
    return calendarRows.filter((row) => !isOffDayWithoutAttendance(row, getDraft(row)))
  ...
  async function fetchData(resetMessage = true) {
    ...
    if (authError || !authData.user) {
      setErrorMessage('Session user belum ditemukan. Silakan login terlebih dahulu.')
      setLoading(false)
      return
    }
    ...
    if (!employeeData.machine_pin) {
      setLogs([])
      setErrorMessage('Machine PIN belum tersedia pada data employee. Silakan hubungi HR.')
      setLoading(false)
      return
    }
  ...
  function ensureDraft(row: CalendarDayRow) {
    ...
    const draft: RowDraft = {
      ...
      manual_check_in: row.log?.manual_check_in || row.log?.requested_check_in || '',
      manual_check_out: row.log?.manual_check_out || row.log?.requested_check_out || '',
      employee_daily_note: row.log?.employee_daily_note || row.log?.correction_reason || '',
      absence_file: null,
      phl_file: null,
    }
  ...
  function openEdit(row: CalendarDayRow) {
    if (isPeriodLocked) {
      setErrorMessage('Periode ini sudah dikunci HR. Hubungi HR jika perlu revisi.')
      return
    }

    if (submittedPeriod) {
      setErrorMessage('Periode ini sudah disubmit. Tunggu approval atau hubungi HR jika perlu revisi.')
      return
    }
  ...
  function updateRowDraft(date: string, field: keyof RowDraft, value: string | File | null) {
    setRowDrafts((prev) => ({
      ...prev,
      [date]: {
  ...
  function toggleAttendanceSelection(row: CalendarDayRow) {
    ...
    if (isOffDayWithoutAttendance(row, getDraft(row))) return
  ...
  async function handleSubmitPeriod() {
    ...
    if (isPeriodLocked) {
      setErrorMessage('Periode ini sudah dikunci HR. Employee tidak bisa submit atau revisi data sampai HR membuka lock.')
      setSubmittingPeriod(false)
      return
    }

    if (submittedPeriod) {
      setErrorMessage('Periode ini sudah pernah dikirim. Tunggu approval atasan atau HR.')
      setSubmittingPeriod(false)
      return
    }
    ...
    for (const row of selectedRows) {
      ...
      if (draft.absence_file) {
        const upload = await uploadFile(draft.absence_file, 'attendance-absence-proofs')
      ...
      if (draft.phl_file) {
        const upload = await uploadFile(draft.phl_file, 'attendance-phl-proofs')
      ...
      const payload = {
        ...
        absence_request_source: meta.absenceRequestType ? 'employee_attendance_confirmation' : null,

        correction_status: 'pending',
        correction_type: correctionType,
        correction_reason: draft.employee_daily_note || `${meta.label} dari konfirmasi absensi employee.`,
    ...
    const leaveMaturity = getLeaveMaturity(employee, periodRange.start, periodRange.end)
  ...
  function validateRowBeforeSubmit(row: CalendarDayRow) {
    ...
    const hasManualTime = Boolean(draft.manual_check_in || draft.manual_check_out)
    ...
    if ((incomplete || (noMachineData && hasManualTime)) && !draft.employee_daily_note.trim()) {
    ...
    if (meta.requiresProof && !row.log?.absence_proof_url && !draft.absence_file) {
  ...
  function exportCsv() {
    const rows = calendarRows.map((row) => {
      ...
      return {
        ...
        manual_check_in: row.log?.manual_check_in || draft.manual_check_in || '',
        manual_check_out: row.log?.manual_check_out || draft.manual_check_out || '',
        ...
        employee_confirmation_status: row.log?.employee_confirmation_status || '',
      ...
      <section className="space-y-6 p-6">
        ...
        {isPeriodLocked && (
          <div className="rounded-[28px] border border-red-200 bg-red-50 p-5 text-sm leading-6 text-red-700">
            ...
            <p>
              Data absensi periode ini sudah dikunci oleh HR. Employee tidak bisa
              mengubah jam manual, upload bukti, memilih tanggal, atau submit ulang
              sampai HR membuka lock.
            </p>
        ...
        {!isPeriodLocked && periodConfirmation?.unlocked_at && (
          <div className="rounded-[28px] border border-orange-200 bg-orange-50 p-5 text-sm leading-6 text-orange-700">
            ...
            <p>
              HR membuka lock periode ini untuk kebutuhan revisi. Jika revisi sudah
              selesai, periode dapat dikunci kembali oleh HR.
            </p>
            ...
            <>
              <div className="grid gap-5 border-b border-black/5 p-6 xl:grid-cols-[0.75fr_1fr]">
                <div className="rounded-[28px] border border-black/5 bg-[#1d1d1f] p-6 text-white">
                  ...
                  <div className="mt-5 grid gap-3">
                    <ProfileRow label="Machine PIN" value={employee?.machine_pin || '-'} />
                    <ProfileRow label="Jabatan" value={employee?.position || '-'} />
                    <ProfileRow label="Periode" value={getPeriodLabel(periodMonth)} />
                    <ProfileRow label="Status Lock" value={isPeriodLocked ? 'Locked by HR' : 'Unlocked'} />
                  </div>
                </div>
                ...
                <div className="rounded-[28px] border border-black/5 bg-white/70 p-6 shadow-sm">
                  ...
                  <p className="mt-1 text-sm leading-6 text-[#6e6e73]">
                    Klik tombol <strong>Lengkapi</strong> untuk menambahkan jam manual,
                    keterangan ketidakhadiran, jenis cuti, klaim PHL, bukti, atau catatan lainnya.
                  </p>
                  ...
                  {latestLog ? (
                    <div className="mt-5 grid gap-4 md:grid-cols-4">
                      <MiniInfoCard title="Tanggal" value={formatDisplayDate(latestLog.attendance_date)} />
                      <MiniInfoCard title="Check In" value={latestLog.check_in || '-'} />
                      <MiniInfoCard title="Check Out" value={latestLog.check_out || '-'} />
                      <MiniInfoCard title="Status" value={formatStatus(latestLog.status || 'present', latestLog)} />
                    </div>
                  ) : (
                    <div className="mt-5 rounded-[24px] border border-dashed border-black/10 bg-[#f5f5f7]/70 p-6 text-sm text-[#6e6e73]">
                      Belum ada data absensi pada periode ini, tetapi employee tetap bisa menambahkan keterangan manual per tanggal selama periode belum dikunci atau belum disubmit.
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-4 border-b border-black/5 bg-[#f5f5f7]/70 p-6 xl:flex-row xl:items-center xl:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm leading-6 text-[#6e6e73]">
                    Centang hari yang sudah dicek. Hari tanpa data tetap bisa diberi keterangan manual lewat tombol Lengkapi.
                  </p>

                  <p className="mt-1 text-xs font-semibold text-[#007aff]">
                    Dipilih: {selectedRows.length} dari {selectableRows.length} hari yang perlu dikonfirmasi.
                  </p>

                  {isReadOnlyPeriod && (
                    <p className="mt-2 text-xs font-bold text-orange-700">
                      Tombol konfirmasi nonaktif karena periode sudah disubmit atau dikunci.
                    </p>
                  )}
                </div>
                ...
                <div className="flex flex-col gap-3 md:flex-row md:items-center">
                  <button
                    ...
                    disabled={selectableRows.length === 0 || isReadOnlyPeriod || submittingPeriod}
                  ...
                  <button
                    ...
                    disabled={selectedRows.length === 0 || isReadOnlyPeriod || submittingPeriod}
                    className="harmony-button-primary disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    <Send size={18} />
                    {submittingPeriod ? 'Mengirim...' : 'Submit Periode ke Atasan'}
                  </button>
                </div>
              </div>
                ...
                <table className="w-full min-w-[1560px] border-collapse text-left text-sm">
                  <thead>
                    <tr className="border-b border-black/5 bg-[#f5f5f7]/90 text-xs uppercase tracking-wide text-[#6e6e73]">
                      <th className="w-[5%] px-5 py-4 text-center font-semibold">Cek</th>
                      <th className="w-[10%] px-5 py-4 font-semibold">Tanggal</th>
                      <th className="w-[8%] px-5 py-4 font-semibold">Hari</th>
                      <th className="w-[9%] px-5 py-4 font-semibold">Clock In</th>
                      <th className="w-[9%] px-5 py-4 font-semibold">Clock Out</th>
                      <th className="w-[8%] px-5 py-4 font-semibold">Durasi</th>
                      <th className="w-[11%] px-5 py-4 font-semibold">Status</th>
                      <th className="w-[13%] px-5 py-4 font-semibold">Keterangan</th>
                      <th className="w-[12%] px-5 py-4 font-semibold">Validasi</th>
                      <th className="w-[8%] px-5 py-4 font-semibold">Approval</th>
                      <th className="w-[12%] px-5 py-4 text-center font-semibold">Action</th>
                    </tr>
                  </thead>
                  ...
                  <tbody>
                    {calendarRows.map((row) => {
                      ...
                      const isOffDayNoAttendance = isOffDayWithoutAttendance(row, draft)
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-5 py-4">
                            <TimeCell
                              value={row.log?.check_in || draft.manual_check_in || '-'}
                              manual={Boolean(draft.manual_check_in || row.log?.manual_check_in)}
                            />
                          </td>

                          <td className="px-5 py-4">
                            <TimeCell
                              value={row.log?.check_out || draft.manual_check_out || '-'}
                              manual={Boolean(draft.manual_check_out || row.log?.manual_check_out)}
                            />
                          </td>
                          ...
                          <td className="px-5 py-4">
                            <ApprovalBadge status={row.log?.supervisor_approval_status || 'none'} />
                          </td>
        ...
        {editOpen && selectedRow && (
          <EditAttendanceModal
            ...
            onChange={(field, value) => updateRowDraft(selectedRow.date, field, value)}
            onClose={closeEdit}
          />
        )}
      </section>
        ...
        <div className="space-y-5 overflow-y-auto p-6">
          {locked && (
            <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm leading-6 text-red-700">
              Periode ini sudah dikunci HR. Data hanya bisa dilihat dan tidak bisa diubah.
            </div>
          )}

          <div className="grid gap-4 md:grid-cols-3">
            <ReadOnlyBox label="Data Mesin Masuk" value={row.log?.check_in || '-'} />
            <ReadOnlyBox label="Data Mesin Pulang" value={row.log?.check_out || '-'} />
            <ReadOnlyBox label="Status Awal" value={formatStatus(row.status, row.log)} />
          </div>
          ...
          <label className="block">
            ...
            <select
              ...
              disabled={locked}
              onChange={(event) => onChange('daily_type', event.target.value as DailyType)}
              className="harmony-select disabled:cursor-not-allowed disabled:opacity-60"
            >
              <option value="present">Hadir Normal</option>
              <option value="manual_attendance">Hadir Manual / Koreksi Jam</option>
          ...
          <div className="rounded-[22px] border border-black/5 bg-[#f5f5f7]/80 p-4 text-sm leading-6 text-[#6e6e73]">
            ...
            <p className="mt-1">
              {getDailyTypeDescription(draft.daily_type)}
            </p>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <label className="block">
              ...
              <input
                ...
                disabled={locked}
                onChange={(event) => onChange('manual_check_in', event.target.value)}
            ...
            <label className="block">
              ...
              <input
                ...
                disabled={locked}
                onChange={(event) => onChange('manual_check_out', event.target.value)}
                className="harmony-input disabled:cursor-not-allowed disabled:opacity-60"
              />
            </label>
          </div>
          ...
          <label className="block">
            ...
            <textarea
              ...
              disabled={locked}
              onChange={(event) => onChange('employee_daily_note', event.target.value)}
          ...
          {(incomplete || noMachineData || meta.isAbsenceLike || meta.requiresProof) && (
            <FilePickerBox
...
function AttendanceCheckButton({
  ...
  const canConfirm = !isOffDayWithoutAttendance(row, draft)

  if (!canConfirm) {
    return (
      <div
        className="mx-auto flex h-9 w-9 items-center justify-center rounded-2xl border border-black/5 bg-[#f5f5f7] text-[#c7c7cc]"
        title="Off day tanpa data absensi tidak perlu dikonfirmasi."
      >
...
function RequestLabelBadge({
  ...
  const label =
    ...
    getDailyTypeMeta(draft.daily_type).label
...
function ValidationInfo({
  ...
  if (phl) {
    return (
      <div className="space-y-1">
        <p className="text-xs font-bold text-[#7b2cbf]">
          Potensi PHL
        </p>
  ...
  if (meta.isAbsenceLike) {
    return (
      <div className="space-y-1">
        <p className="text-xs font-bold text-[#0059b8]">
          {meta.label}
        </p>
        <p className="text-xs leading-5 text-[#6e6e73]">
  ...
  return (
    <div>
      <div className="flex items-center gap-2 font-semibold text-[#1d1d1f]">
        <Timer size={14} className={manual ? 'text-[#007aff]' : 'text-[#86868b]'} />
        {value}
      </div>

      {manual && (
        <p className="mt-1 text-[11px] font-bold text-[#007aff]">
          Manual
        </p>
      )}
    </div>
...
function isOffDayWithoutAttendance(row: CalendarDayRow, draft?: RowDraft) {
  ...
  return (row.is_weekend || Boolean(row.holiday_name)) && !hasAttendance && !hasAbsenceInfo
}
...
function inferDailyType(row: CalendarDayRow): DailyType {
  ...
  if (row.log?.manual_check_in || row.log?.manual_check_out) return 'manual_attendance'
...
function calculatePeriodTotals(
  ...
  calendarRows.forEach((row) => {
    ...
    const isHoliday = holidays.some((holiday) => holiday.holiday_date === row.date)
...
function formatPeriodStatus(period: AttendancePeriodConfirmation | null) {
  ...
  if (period.hr_status === 'ready_for_hr') return 'Sudah disetujui atasan dan siap diproses HR.'
  if (period.supervisor_status === 'approved') return 'Sudah disetujui atasan dan menunggu HR.'
  if (period.supervisor_status === 'rejected') return 'Ditolak atasan. Perlu revisi employee.'
  if (period.supervisor_status === 'pending') return 'Sudah dikirim dan sedang menunggu approval atasan.'
  if (period.employee_status === 'submitted') return 'Sudah disubmit oleh employee.'
