...
export default function EmployeeLeavePostponePage() {
  ...
  const computed = useMemo(() => {
    ...
    const anniversaryDate = addDays(oldCycleEnd, 1)
    const deadline = subtractDays(oldCycleEnd, 7)
    const expiredAt = addMonths(anniversaryDate, 6)
  ...
  async function fetchData() {
    ...
    try {
      ...
      const response = await fetch('/api/employee/leave/postpone', {
        method: 'GET',
        headers: {
      ...
      if (!response.ok || result?.success === false) {
        throw new Error(
          result?.error || result?.message || 'Gagal memuat data postpone.',
        )
      }

      const currentEmployee = (result?.employee || null) as Employee | null
      const cycleRows = (result?.cycles || []) as AnnualLeaveCycle[]
      const requestRows = (result?.requests || []) as LeavePostponeRequest[]

      setEmployee(currentEmployee)
      setCycles(cycleRows)
      setRequests(requestRows)

      const firstEligible = cycleRows.find((cycle) => {
        if (!cycle.cycle_end || Number(cycle.remaining_days || 0) <= 0) {
          return false
        }

        const deadline = subtractDays(cycle.cycle_end, 7)
        const hasActiveRequest = requestRows.some(
          (item) =>
            item.source_cycle_id === cycle.id &&
            !['rejected', 'cancelled'].includes(
              String(item.approval_status || '').toLowerCase(),
            ),
        )

        return isBeforeOrSame(todayISO(), deadline) && !hasActiveRequest
      })

      setSelectedCycleId((current) =>
        current && cycleRows.some((cycle) => cycle.id === current)
          ? current
          : firstEligible?.id || cycleRows[0]?.id || '',
      )
    } catch (error: any) {
      setEmployee(null)
      setCycles([])
      ...
      setMessage({
        type: 'error',
        text: error?.message || 'Gagal memuat data postpone cuti.',
      })
    } finally {
      setLoading(false)
    }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok || result?.success === false) {
        throw new Error(result?.error || result?.message || 'Gagal membuat pengajuan postpone.')
      }
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-[#6e6e73]">
                ...
                Jika disetujui HR, sisa cuti berlaku sampai 6 bulan setelah anniversary cuti berikutnya. Contoh anniversary 25 Februari → expiry 25 Agustus.
              </p>
            </div>
            ...
            {loading ? (
              ...
              <form onSubmit={handleSubmit} className="mt-6 space-y-4">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <InfoBox label="Anniversary Baru" value={formatDate(computed.anniversaryDate)} />
