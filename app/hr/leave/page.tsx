...
export default function HRLeavePage() {
  ...
  const [leaveBalanceLifecycle, setLeaveBalanceLifecycle] =
    useState<LeaveBalanceLifecycleSummary[]>([])
  ...
  const filteredLeaveBalanceLifecycle = useMemo(() => {
    const keyword = searchKeyword.trim().toLowerCase()

    if (!keyword) return leaveBalanceLifecycle

    return leaveBalanceLifecycle.filter((item) => {
      return (
        item.full_name?.toLowerCase().includes(keyword) ||
        item.employee_number?.toLowerCase().includes(keyword) ||
        item.department?.toLowerCase().includes(keyword) ||
  ...
  }, [leaveBalanceLifecycle, searchKeyword])

  const totalRegularLeaveDays = useMemo(() => {
    return leaveBalanceLifecycle.reduce(
      (sum, item) => sum + Number(item.annual_regular_days || 0),
      0
    )
  }, [leaveBalanceLifecycle])

  const totalActivePostponeDays = useMemo(() => {
    return leaveBalanceLifecycle.reduce(
      (sum, item) => sum + Number(item.postpone_active_days || 0),
      0
    )
  }, [leaveBalanceLifecycle])

  const totalExpiredPostponeDays = useMemo(() => {
    return leaveBalanceLifecycle.reduce(
      (sum, item) => sum + Number(item.postpone_expired_days || 0),
      0
    )
  }, [leaveBalanceLifecycle])

  const totalManualPostponeNetDays = useMemo(() => {
    return leaveBalanceLifecycle.reduce(
      (sum, item) => sum + Number(item.postpone_manual_net_days || 0),
      0
    )
  }, [leaveBalanceLifecycle])
  ...
  async function fetchData(options?: { preserveMessages?: boolean }) {
    ...
    if (lifecycleError) {
      console.warn('Leave lifecycle reconcile warning:', lifecycleError.message)
    }
  ...
  async function fetchLeaveBalanceLifecycle() {
    ...
    if (error) {
      ...
      setLeaveBalanceLifecycle([])
      return
    }

    setLeaveBalanceLifecycle(
      (data || []) as LeaveBalanceLifecycleSummary[]
    )
  }
  ...
  return (
    <>
      <Topbar
        title="Cuti, Izin & PHL"
        description="Kelola pengajuan, cuti tahunan matang, postpone/carry forward, PHL, dan lifecycle saldo karyawan."
      />
          ...
          <div className="relative flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
            <div>
              ...
              <p className="mt-5 max-w-2xl text-sm leading-7 text-white/62">
                Review pengajuan, pisahkan cuti tahunan matang dari postpone, pantau expiry otomatis, dan kelola PHL dalam satu halaman.
              </p>
            </div>
...
function LeaveBalanceLifecycleTab({
  rows,
  totalRegularDays,
  totalPostponeDays,
  totalExpiredPostponeDays,
  totalManualPostponeNetDays,
}: {
  rows: LeaveBalanceLifecycleSummary[]
  totalRegularDays: number
  totalPostponeDays: number
  totalExpiredPostponeDays: number
  totalManualPostponeNetDays: number
}) {
  const totalAvailable = rows.reduce(
    (sum, item) => sum + Number(item.annual_total_available_days || 0),
    0
  )

  const totalPHL = rows.reduce(
    (sum, item) => sum + Number(item.phl_total_available_days || 0),
    0
  )
  ...
  return (
    <div>
      <SectionIntro
        title="Saldo Cuti Tahunan, Postpone & PHL"
        description="Cuti matang, postpone aktif, postpone expired, adjustment HR, dan PHL ditampilkan sebagai sumber berbeda. Postpone expired tetap menjadi histori tetapi otomatis tidak dapat digunakan."
      />

      <div className="grid gap-3 border-b border-black/5 p-5 sm:grid-cols-2 xl:grid-cols-6">
        <MiniMetricCard
          label="Cuti Matang"
          value={`${formatNumber(totalRegularDays)} hari`}
          tone="blue"
        />
        <MiniMetricCard
          label="Postpone Aktif"
          value={`${formatNumber(totalPostponeDays)} hari`}
          tone="purple"
        />
        <MiniMetricCard
          label="Postpone Manual Net"
          value={`${formatSignedNumber(totalManualPostponeNetDays)} hari`}
          tone="blue"
        />
        <MiniMetricCard
          label="Postpone Expired"
          value={`${formatNumber(totalExpiredPostponeDays)} hari`}
          tone="red"
        />
        <MiniMetricCard
          label="Total Cuti Tersedia"
          value={`${formatNumber(totalAvailable)} hari`}
          tone="green"
        />
        <MiniMetricCard
          label="Total PHL Aktif"
          value={`${formatNumber(totalPHL)} hari`}
          tone="orange"
        />
      </div>
      ...
      {rows.length === 0 ? (
        <EmptyState
          title="Ringkasan saldo belum tersedia"
          description="Jalankan SQL lifecycle final lalu Refresh. Cycle matang akan dibuat otomatis berdasarkan anniversary."
        />
      ) : (
        <>
          <div className="grid gap-3 p-4 2xl:hidden">
            {rows.map((item) => (
              <div
                ...
                className="rounded-[26px] border border-black/5 bg-white p-4 shadow-sm"
              >
                ...
                <div className="mt-4 grid grid-cols-2 gap-3">
                  <BalanceBox
                    label="Cuti Matang"
                    value={`${formatNumber(item.annual_regular_days)} hari`}
                  />
                  <BalanceBox
                    label="Postpone Aktif"
                    value={`${formatNumber(item.postpone_active_days)} hari`}
                  />
                  <BalanceBox
                    label="Postpone Manual Net"
                    value={`${formatSignedNumber(item.postpone_manual_net_days)} hari`}
                  />
                  <BalanceBox
                    label="Adjustment HR"
                    value={`${formatSignedNumber(item.annual_manual_adjustment_days)} hari`}
                  />
                  <BalanceBox
                    label="Total Tersedia"
                    value={`${formatNumber(item.annual_total_available_days)} hari`}
                    strong
                  />
                  <BalanceBox
                    label="Postpone Expired"
                    value={`${formatNumber(item.postpone_expired_days)} hari`}
                  />
                  <BalanceBox
                    label="PHL Aktif"
                    value={`${formatNumber(item.phl_total_available_days)} hari`}
                  />
                </div>

                <div className="mt-3 rounded-2xl bg-[#f5f5f7]/80 p-3 text-xs leading-5 text-[#6e6e73]">
                  <p>
                    Matang terakhir: <strong>{formatDisplayDate(item.latest_matured_at || '')}</strong>
                  </p>
                  <p>
                    Expiry postpone berikutnya: <strong>{formatDisplayDate(item.next_postpone_expiry || '')}</strong>
                  </p>
                  <p>
                    Expiry PHL berikutnya: <strong>{formatDisplayDate(item.next_phl_expiry || '')}</strong>
                  </p>
                </div>
              </div>
            ))}
          ...
          <div className="hidden overflow-x-auto 2xl:block">
            <table className="min-w-[1580px] w-full border-collapse text-left text-[12px]">
              <thead>
                <tr className="border-b border-black/5 bg-[#f5f5f7]/90 text-xs uppercase tracking-wide text-[#6e6e73]">
                  <th className="px-4 py-3">Karyawan</th>
                  <th className="px-4 py-3">Cuti Matang</th>
                  <th className="px-4 py-3">Postpone Aktif</th>
                  <th className="px-4 py-3">Manual Postpone Net</th>
                  <th className="px-4 py-3">Expiry Postpone</th>
                  <th className="px-4 py-3">Adjustment HR</th>
                  <th className="px-4 py-3">Total Tersedia</th>
                  <th className="px-4 py-3">Postpone Expired</th>
                  <th className="px-4 py-3">PHL Aktif</th>
                  <th className="px-4 py-3">Expiry PHL</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-4 align-top font-bold text-[#0059b8]">
                      {formatNumber(item.annual_regular_days)} hari
                    </td>
                    <td className="px-4 py-4 align-top font-bold text-[#7b2cbf]">
                      {formatNumber(item.postpone_active_days)} hari
                    </td>
                    <td className="px-4 py-4 align-top font-semibold text-[#0059b8]">
                      {formatSignedNumber(item.postpone_manual_net_days)} hari
                    </td>
                    <td className="px-4 py-4 align-top">
                      {formatDisplayDate(item.next_postpone_expiry || '')}
                    </td>
                    <td className="px-4 py-4 align-top">
                      {formatSignedNumber(item.annual_manual_adjustment_days)} hari
                    </td>
                    <td className="px-4 py-4 align-top">
                      <span className="inline-flex rounded-full bg-green-50 px-3 py-1 font-bold text-green-700">
                        {formatNumber(item.annual_total_available_days)} hari
                      </span>
                    </td>
                    <td className="px-4 py-4 align-top text-red-700">
                      {formatNumber(item.postpone_expired_days)} hari
                    </td>
                    <td className="px-4 py-4 align-top">
                      {formatNumber(item.phl_total_available_days)} hari
                    </td>
                    <td className="px-4 py-4 align-top">
                      {formatDisplayDate(item.next_phl_expiry || '')}
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <div className="border-t border-black/5 bg-[#f8fbff] p-5 text-xs leading-6 text-[#41607d]">
        <strong>Lifecycle otomatis:</strong> anniversary membuat cycle 12 hari yang sudah matang.
        Sisa cycle lama yang disetujui sebagai postpone berlaku mulai anniversary berikutnya dan
        berakhir tepat 6 bulan setelah anniversary. Setelah lewat expiry, saldo postpone otomatis
        keluar dari saldo aktif tanpa menghapus histori. PHL juga otomatis tidak dihitung setelah
        expired_at. Postpone manual dan adjustment HR ditampilkan terpisah untuk audit.
      </div>
    </div>
  )
}
  ...
  return (
    <div className="rounded-2xl bg-[#f5f5f7]/80 p-3">
      ...
      <p
        className={[
          'mt-1 text-sm',
          strong ? 'font-bold text-green-700' : 'font-semibold text-[#1d1d1f]',
        ].join(' ')}
      >
        {value}
      </p>
...
function MiniMetricCard({
  ...
  label: string
  value: string
  tone: 'blue' | 'purple' | 'red' | 'green' | 'orange'
}) {
  const cls = {
  ...
  return (
    <div className={`rounded-[22px] border p-4 ${cls}`}>
...
function formatSignedNumber(value: number | null | undefined) {
  const num = Number(value || 0)
  const text = Number.isInteger(num) ? String(Math.abs(num)) : Math.abs(num).toFixed(2)
        ...
        {summaries.map((item, index) => (
          ...
          >
            ...
            <div className="mt-5 grid grid-cols-3 gap-3">
              <PHLBalanceBox label="Earned" value={item.total_earned_days || 0} tone="green" />
              <PHLBalanceBox label="Used" value={item.total_used_days || 0} tone="orange" />
              <PHLBalanceBox label="Available" value={item.total_available_days || 0} tone="purple" />
            </div>
